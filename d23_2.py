...
def get_all_moves_from_corridor(burrow: Burrow) -> list[Burrow]:
    ...
    for i in range(len(burrow.corridor)):
        ...
        room_ready = True
        for ri in range(4):
            if not (room[ri] == "." or room[ri] == amp):
                room_ready = False
                break
            if ri < 3 and room[ri] == "." and room[ri + 1] == amp:
                room_index = ri
            if ri == 3 and room[ri] == ".":
                room_index = ri
        if not room_ready:
...
def get_all_moves_from_rooms(burrow: Burrow) -> list[Burrow]:
    ...
    for room_name in ["a", "b", "c", "d"]:
        for room_index in range(4):
            ...
            amp = room[room_index]
            all_in_room_ok = True
            for ri in range(room_index, 4):
                if room[ri] != room_name:
                    all_in_room_ok = False
                    break
            if all_in_room_ok:
                break
            ...
            for ci in possible_corridor_indexes:
                ...
                new_bur.last_move_cost = steps * step_cost[amp]
                res.append(new_bur)
            # we can move only the top-most one
            break
    return res
...
def heuristic(burrow: Burrow) -> int:
    ...
    for i in range(len(burrow.corridor)):
        if burrow.corridor[i] == ".":
            continue
        amp = burrow.corridor[i]
        steps = abs(rooms_to_corridor_index[amp] - i) + 3
    ...
    for room_name in ["a", "b", "c", "d"]:
        for room_index in range(4):
            ...
            if amp == ".":
                continue
            steps = abs(rooms_to_corridor_index[room_name] - rooms_to_corridor_index[amp]) + 2
            res += steps * step_cost[amp]
    return res
...
def find_best_cost(burrow: Burrow) -> int:
    goal = Burrow(None, Rooms(["a", "a", "a", "a"],
                              ["b", "b", "b", "b"],
                              ["c", "c", "c", "c"],
                              ["d", "d", "d", "d"]))
...
def run(lines: list[str]) -> int:
    ...
    row2 = lines[4].strip().split("#")
    row3 = lines[5].strip().split("#")
    rooms = Rooms([row0[3].lower(), row1[1].lower(), row2[1].lower(), row3[1].lower()],
                  [row0[4].lower(), row1[2].lower(), row2[2].lower(), row3[2].lower()],
                  [row0[5].lower(), row1[3].lower(), row2[3].lower(), row3[3].lower()],
                  [row0[6].lower(), row1[4].lower(), row2[4].lower(), row3[4].lower()])
